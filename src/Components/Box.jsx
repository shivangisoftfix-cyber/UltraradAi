...
const iconList = [FaStar, FaHeart, FaUser, FaCloud, FaCamera, FaBell];

const Box = () => {
  return (
    <div>
      {/* Feature Boxes Section */}
      <section className="bg-blue-300  h-[100-vh] ">
          ...
          {iconList.map((Icon, index) => (
            ...
            >
              <div className="relative flex items-center justify-center mb-4">
                <span
                  className="absolute w-16 h-16 rounded-full bg-blue-500 opacity-20 scale-0 group-hover:scale-125 transition-transform duration-300"
                ></span>
                <Icon className="text-5xl relative text-white bg-blue-500 border rounded-full p-3" />
              </div>
              <h4 className="text-xl font-bold mb-2">Box {index+1}</h4>
              <p className="text-gray-600">Short description of this feature.</p>
              <span className="absolute bottom-0 left-0 w-0 h-1 bg-blue-500 transition-all duration-300 group-hover:w-full rounded-b-lg"></span>
            </div>
          ))}
        </div>
